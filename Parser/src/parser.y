...
// Helper function to add a new symbol to the table
...
%type <dval> expression conditional_expression
...
declaration_specifiers declarator SEMICOLON
        {
            addSymbol($2, "function prototype");
free($1);
            free($2);
        }
    ;
...
declaration
    : declaration_specifiers { declaratorKind = ""; currentDeclarationType = $1;
    ...
    : declarator
        {
            if (declaratorKind == "array") {
                ...
                addSymbol($1, "function pointer");
} else {
                addSymbol($1, currentDeclarationType);
}
            $$ = $1;
        }
    |
...
compound_type_definition
    : STRUCT IDENTIFIER
      ...
      LBRACE member_list RBRACE
        {
            string ret = string("struct ") + string($2);
            scopeStack.pop();
$$=strdup(ret.c_str());
            free($2);
        }
    | STRUCT LBRACE member_list RBRACE
        {
            string ret = string("struct ") + "anonymous_struct";
$$=strdup(ret.c_str());
        }
    | UNION IDENTIFIER
        {
      ...
      LBRACE member_list RBRACE
        {
            string ret = string("union ") + string($2);
            scopeStack.pop();
$$=strdup(ret.c_str());
            free($2);
        }
    | UNION LBRACE member_list RBRACE
        {
            string ret = string("union ") + "anonymous_union";
$$=strdup(ret.c_str());
        }
    | ENUM IDENTIFIER LBRACE enum_list RBRACE
        {
            string ret = string("enum ") + string($2);
$$=strdup(ret.c_str());
            free($2);
        }
    | ENUM LBRACE enum_list RBRACE
        {
            string ret = string("enum ") + "anonymous_enum";
$$=strdup(ret.c_str());
        }
    ;
parameter_list
